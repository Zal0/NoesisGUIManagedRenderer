...
void main()
{
    ...
    {
        vec2 _123 = vec2(Buffer2.blurSize * Buffer1.textureSize.z, 0.0) * _87[_117];
        vec2 _128 = in_var_TEXCOORD1 - vec2(Buffer2.shadowOffset.x * Buffer1.textureSize.z, Buffer2.shadowOffset.y * Buffer1.textureSize.w);
    ...
    vec4 _149 = texture(SPIRV_Cross_CombinedimageimageSampler, clamp(in_var_TEXCOORD1, in_var_TEXCOORD2.xy, in_var_TEXCOORD2.zw));
    out_var_SV_Target0 = (_149 + ((Buffer2.shadowColor * _114) * (1.0 - _149.w))) * (Buffer0.opacity * texture(SPIRV_Cross_CombinedpatternpatternSampler, in_var_TEXCOORD0).w);
}
